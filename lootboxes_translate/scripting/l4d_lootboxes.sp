...
#define SI_CHANCES			"100.0,100.0,100.0,100.0,100.0,100.0"			// 6 values, one for each special
...
#define SI_CHANCES_1		"100.0,100.0,100.0"
...
{
	LoadTranslations("l4d_lootboxes.phrases");
	...
	g_hTankDrops =		CreateConVar("l4d_lootbox_tank_drops",				"2,5",				"Min and max amount of lootboxes dropped when a tank dies.\n1|2 values, separated by commas, no spaces.\nIf 1 value is placed, max and min values will be the same.", FCVAR_FLAGS);
	g_hWitchDrops =		CreateConVar("l4d_lootbox_witch_drops",				"1,5",				"Min and max amount of lootboxes dropped when a witch dies.\n1|2 values, separated by commas, no spaces.\nIf 1 value is placed, max and min values will be the same.", FCVAR_FLAGS);
...
Action AdminSpawnBox(int client, int args)		// Admin can spawn lootboxes at player position DISABLED
{
	...
	{
		CPrintToChat(client, "%t", "nospawn", CHAT_TAG);
		return Plugin_Handled;
	}
	vPos[2] += 8.0;
	int iCount;
...
Action PlayerSpeed_Timer(Handle timer, int client)
{
	...
	CPrintToChat(client, "%t", "Speedboost3", CHAT_TAG);
	return Plugin_Continue;
}
...
Action PlayerInvul_Timer(Handle timer, int client)
{
	...
	EmitSoundToClient(client, SND_BOOST_END);
	CPrintToChat(client, "%t", "invulnerability2", CHAT_TAG);
	return Plugin_Continue;
}
...
Action PlayerRegen_Timer(Handle timer, int client)
{
	...
	{
		CPrintToChat(client, "%t", "regen3", CHAT_TAG);
...
Action PlayerReverse_Timer(Handle timer, int client)
{
	...
	CPrintToChat(client, "%t", "revers3", CHAT_TAG);
	EmitSoundToClient(client, SND_BOOST_END);
	return Plugin_Continue;
}
...
Action PlayerExpl_Timer(Handle timer, int client)
{
	...
	CPrintToChat(client, "%t", "explshot3", CHAT_TAG);
	EmitSoundToClient(client, SND_BOOST_END);
	return Plugin_Continue;
}
...
Action PlayerFragile_Timer(Handle timer, int client)
{
	...
	CPrintToChat(client, "%t", "Fragility4", CHAT_TAG);
...
Action PlayerAngles_Timer(Handle timer, int client)
{
	...
	if( g_iPlayerBoosts[client] & PN_ANGLES )
	{
		...
		if( fTime <= 5.0 )
		{
			...
			CPrintToChat(client, "%t", "ranang3", CHAT_TAG);
			EmitSoundToClient(client, SND_BOOST_END);	
		}
		else
		{
...
Action LootBox_Used(int entity, int caller, int activator, UseType type, float value)
{
	...
	if( g_fPlayerUse[activator] + 3.0 > GetGameTime() )
	{
		...
		{
			CPrintToChat(activator, "%t", "waitact", CHAT_TAG, 3 - GetGameTime() + g_fPlayerUse[activator]);	//"%s Wait %.1f seconds to open next LootBox."
...
bool SPawnT1(int client, float vPos[3])
{
	...
	CPrintToChat(client, "%t", "Weapon1t", CHAT_TAG);
...
bool SpawnT2(int client, float vPos[3])
{
	...
	TeleportEntity(iWeapon, vPos, NULL_VECTOR, NULL_VECTOR);
	CPrintToChat(client, "%t", "Weapon2t", CHAT_TAG);
	EmitAmbientSound(SND_GOOD_OPEN, vPos);
	return true;
}
...
bool SpawnT3(int client, float vPos[3])
{
	...
	if( GetRandomInt(0, 1) == 0 )
	{
		...
		TeleportEntity(iWeapon, vPos, NULL_VECTOR, NULL_VECTOR);
		CPrintToChat(client, "%t", "M60found", CHAT_TAG);
	}
	else
	{
		...
		TeleportEntity(iWeapon, vPos, NULL_VECTOR, NULL_VECTOR);
		CPrintToChat(client, "%t", "Grenlaynchfound", CHAT_TAG);
	}
	EmitAmbientSound(SND_GOOD_OPEN, vPos);
	return true;
}
...
bool SpawnSecondary(int client, float vPos[3])
{
	...
	CPrintToChat(client, "%t", "Secfound", CHAT_TAG);
	EmitAmbientSound(SND_GOOD_OPEN, vPos);
	return true;
}
...
bool SpawnDrugs(int client, float vPos[3])
{
	...
	CPrintToChat(client, "%t", "Drugsfound", CHAT_TAG);
	EmitAmbientSound(SND_GOOD_OPEN, vPos);
	return true;
}
...
bool SpawnMeds(int client, float vPos[3])
{
	...
	CPrintToChat(client, "%t", "Medicfound", CHAT_TAG);
	EmitAmbientSound(SND_GOOD_OPEN, vPos);
	return true;
}
...
bool SpawnThrowable(int client, float vPos[3])
{
	...
	TeleportEntity(iWeapon, vPos, NULL_VECTOR, NULL_VECTOR);
	CPrintToChat(client, "%t", "Throwfound", CHAT_TAG);
	EmitAmbientSound(SND_GOOD_OPEN, vPos);
	return true;
}
...
bool SpawnItem(int client, float vPos[3])
{
	...
	if( iEntity != -1 )
	{
		...
		Format(sName, sizeof(sName), "%t", sName);
		CPrintToChat(client, "%t", "Itemfound", CHAT_TAG, sName);
		return true;
	}
	return false;
}
...
bool SpawnUpgrade(int client, float vPos[3])
{
	...
	TeleportEntity(iWeapon, vPos, NULL_VECTOR, NULL_VECTOR);
	CPrintToChat(client, "%t", "Upgfound", CHAT_TAG);
	EmitAmbientSound(SND_GOOD_OPEN, vPos);
	return true;
}
...
bool SpawnLaser(int client, float vPos[3])
{
	...
	for( int i = 1; i <= MaxClients; i++ )
	{
		if( i != client && IsClientInGame(i) && !IsFakeClient(i) )
			CPrintToChat(i, "%t", "Laser2found", CHAT_TAG, client);
	}
	CPrintToChat(client, "%t", "Laserfound", CHAT_TAG);
	return true;
}
...
void GivePlayerSpeed(int client)
{
	if( CheckPlayerBoost(client, PB_SPEED) )
	{
		...
		CPrintToChat(client, "%t", "Speedboost", CHAT_TAG);
		return;
	}
	...
	g_hPlayerSpeedTimer[client] = CreateTimer(g_fBoostTimes[0], PlayerSpeed_Timer, client);
	g_iPlayerBoosts[client] |= PB_SPEED;
	CPrintToChat(client, "%t", "Speedboost2", CHAT_TAG);
...
void GivePlayerInvulnerability(int client)
{
	if( CheckPlayerBoost(client, PB_INVUL) )
	{
		...
		CPrintToChat(client, "%t", "invulnerability", CHAT_TAG);
		return;
	}
...
void GivePlayerRegen(int client)
{
	...
	{
		CPrintToChat(client, "%t", "regen", CHAT_TAG);
		return;		
	}
	...
	g_iPlayerBoosts[client] = PB_REGEN;
	CPrintToChat(client, "%t", "regen2", CHAT_TAG);
...
void GivePlayerFire(int client)
{
	if( CheckPlayerBoost(client, PB_FIRE) )
	{
		...
		CPrintToChat(client, "%t", "firepower", CHAT_TAG);
		return;
	}
	...
	g_hPlayerFireTimer[client] = CreateTimer(g_fBoostTimes[3], PlayerFire_Timer, client);
	g_iPlayerBoosts[client] = PB_FIRE;
	CPrintToChat(client, "%t", "firepower2", CHAT_TAG);
...
void GivePlayerInfAmmo(int client)
{
	if( CheckPlayerBoost(client, PB_IAMMO) )
	{
		...
		CPrintToChat(client, "%t", "infammo", CHAT_TAG);
		return;
	}
	
	g_hPlayerAmmoTimer[client] = CreateTimer(g_fBoostTimes[4], PlayerInfAmmo_Timer, client);
	g_iPlayerBoosts[client] = PB_IAMMO;
	CPrintToChat(client, "%t", "infammo2", CHAT_TAG);
}
...
void GivePlayerExplosive(int client)
{
	if( CheckPlayerBoost(client, PB_EXPL) )
	{
		...
		CPrintToChat(client, "%t", "explshot", CHAT_TAG);
	}
	else
	{
		g_iPlayerBoosts[client] |= PB_EXPL;
		g_hPlayerExplTimer[client] = CreateTimer(g_fBoostTimes[5], PlayerExpl_Timer, client);
		CPrintToChat(client, "%t", "explshot2", CHAT_TAG);
	}
}
...
void WipeHalfZombies(int client)
{
	...
	for( int i = 1; i <= MaxClients; i++ )
	{
		...
		{
			CPrintToChat(i, "%t", "infgaul", CHAT_TAG, client);
		}
	}
	EmitSoundToAll(SND_THANOS);
	CPrintToChat(client, "%t", "infgaul2", CHAT_TAG);
}
...
bool RespawnClient(int client, float vPos[3])
{
	...
	for( int i = 1; i < MaxClients; i++ )
	{
		...
		if( i == client )
			CPrintToChat(i, "%t", "respa", CHAT_TAG, deadSurvivors[rnd]);
		else if( i == deadSurvivors[rnd] )
			CPrintToChat(i, "%t", "respa2", CHAT_TAG, client);
		else
			CPrintToChat(i, "%t", "respa3", CHAT_TAG, client, deadSurvivors[rnd]);
	}
...
void SpawnMob(int client, bool mega)
{
	if( mega )
	{
		...
		{
			if( i != client && IsClientInGame(i) && !IsFakeClient(i) )
				CPrintToChat(i, "%t", "panic1", CHAT_TAG, client);
		}
		CPrintToChat(client, "%t", "panic2", CHAT_TAG);
	...
	else
	{
		...
		{
			if( i != client && IsClientInGame(i) && !IsFakeClient(i) )
				CPrintToChat(i, "%t", "somezomb1", CHAT_TAG, client);
		}
		CPrintToChat(client, "%t", "somezomb2", CHAT_TAG);
	}
...
void BoxTrap(int client, float vPos[3], bool isVomit)
{
	...
	for( int i = 1; i <= MaxClients; i++ )
	{
		...
		//if( client != i && !IsFakeClient(i) )
			//CPrintToChat(i, "%t", "trapopen", CHAT_TAG, client, isVomit ? "a \x03vomit trap\x01." : "an \x03ice trap\x01.");
		...
		if( isVomit )
		{
			...
			CPrintToChatAll("%t", "vomittr", CHAT_TAG, client);
			return;
		}
		else
		{	
			SU_AddFreeze(i, g_hFreezeTime.FloatValue);
			CPrintToChatAll("%t", "icetr", CHAT_TAG, client);
			return;
		}
	}
}
...
void ReverseControls(int client)
{
	if( g_iPlayerBoosts[client] & PN_REVERSE)
	{
		...
		CPrintToChat(client, "%t", "revers1", CHAT_TAG);
		return;
	}
	g_hPlayerReverseTimer[client] = CreateTimer(g_fNerfTimes[0], PlayerReverse_Timer, client);
	CPrintToChat(client, "%t", "revers2", CHAT_TAG);
...
void SpitTrap(int client, float vPos[3])
{
	...
	{
		if( IsClientInGame(i) && i != client && !IsFakeClient(i) )
			CPrintToChat(i,  "%t", "SpitTrap1", CHAT_TAG, client);
	}
	CPrintToChat(client,  "%t", "SpitTrap2", CHAT_TAG);
}
...
bool SpawnWitch(int client, float vPos[3])
{
	...
	{
		if( IsClientInGame(i) && i != client && !IsFakeClient(i) )
			CPrintToChat(i,"%t", "wicthbox1", CHAT_TAG, client);
	}
	CPrintToChat(client,"%t", "wicthbox2", CHAT_TAG);
	return true;
}
...
bool SpawnTank(int client, float vPos[3])
{
	...
	{
		if( IsClientInGame(i) && i != client && !IsFakeClient(i) )
			CPrintToChat(i, "%t", "tankbox1", CHAT_TAG, client);
	}
	CPrintToChat(client, "%t", "tankbox2", CHAT_TAG);
	return true;
}
...
bool ToxicCloud(int client, float vPos[3])
{
	...
	{
		if( IsClientInGame(i) && i != client && !IsFakeClient(i) )
			CPrintToChat(i, "%t", "toxicbox1", CHAT_TAG, client);
	}
	...
	CPrintToChat(client, "%t", "toxicbox2", CHAT_TAG);
	return true;
}
...
bool JockeyRide(int client)
{
	...
	if( result > 0 )
	{
		CPrintToChat(client, "%t", "jocbox", CHAT_TAG);
		return true;
	}
	return false;
}
...
bool SpawnExplBarrel(int client, float vPos[3])
{
	...
	{
		if( IsClientInGame(i) && i != client && !IsFakeClient(i) )
			CPrintToChat(i, "%t", "barelbox1", CHAT_TAG, client);
	}
	CPrintToChat(client, "%t", "barelbox2", CHAT_TAG);
	return true;
}
...
void BlackWhite(int client)
{
	...
	CPrintToChat(client, "%t", "bwbox2", CHAT_TAG);
} 
...
void GiveFragility(int client)
{
	...
	if( g_iPlayerBoosts[client] & PB_INVUL )
	{
		...
		SDKUnhook(client, SDKHook_OnTakeDamage, Invulnerability_Callback);
		EmitSoundToClient(client, SND_BAD_OPEN);
		CPrintToChat(client, "%t", "Fragility1", CHAT_TAG);
	}
	else if( g_iPlayerBoosts[client] & PN_FRAGILE )
	{
		...
		EmitSoundToClient(client, SND_BAD_OPEN);
		CPrintToChat(client, "%t", "Fragility2", CHAT_TAG);
	}
	else
	{
		g_iPlayerBoosts[client] |= PN_FRAGILE;
		...
		EmitSoundToClient(client, SND_BAD_OPEN);
		CPrintToChat(client, "%t", "Fragility3", CHAT_TAG);
	}
}

void BearTrap(int client, float vPos[3])
{
	CPrintToChat(client, "%t", "Beartr", CHAT_TAG);
...
void RandomAngles(int client)
{
	...
	if( g_iPlayerBoosts[client] & PN_ANGLES )
	{
		CPrintToChat(client, "%t", "ranang1", CHAT_TAG);
		return;
	}
	g_iPlayerBoosts[client] |= PN_ANGLES;
	CPrintToChat(client, "%t", "ranang2",CHAT_TAG);
...
bool SpawnFireworks(int client, float vPos[3])
{
	...
	if( result )
	{
		...
		{
			if( i != client && IsClientInGame(i) && !IsFakeClient(i) )
				CPrintToChat(i, "%t", "fpbox1", CHAT_TAG, client);
		}
		CPrintToChat(client, "%t", "fpbox2", CHAT_TAG);
	}
	return result;
}
...
void MakeTitanZombies(int client)
{
	...
	{
		if( IsClientInGame(i) && i != client && !IsFakeClient(i) )
			CPrintToChat(i, "%t", "tzbox1", CHAT_TAG, client);
	}
	CPrintToChat(client, "%t", "tzbox2", CHAT_TAG);
}
/* ========================================================================================= *
 *                                     Weapon Handling                                       *
 * ========================================================================================= */
