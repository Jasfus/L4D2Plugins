...
// Счетчик игроков сбрасывается при старте карты, так как событие ‘connected’ происходит только при загрузке карты. Во время события ‘round_start’ игроки уже подключены, поэтому их присоединение или выход не вызывает обновления счетчика игроков
...
{   
	...
	{
		g_player_num += 1;			        //Количество игроков
		g_player_killspecial[client] = 0;	//Назначаемый атрибут новому игроку: killspecial
		g_player_headshot[client] = 0;		//Атрибут нового игрока: количество хэдшотов
	}
}
...
Action Timer_DisplayHUDInfo(Handle timer)
{
	...
	if( g_show_list & MAXPLAYER_LIST )
	{
		...
		FormatEx(show_hud.info,sizeof(show_hud.info),"===================\n\
													  ★ Убийства / Хедшоты | Текущие игроки: [%d/%d]★\n\
													  ==================="
...
void DisplayPlayerList()
{
	...
	//Пройтись циклом по списку активных игроков и присвоить/записать им порядковые номера.
	//Использование списка survivor позволяет лучше организовать сортировку, не требуется дополнительная проверка валидности игроков в функции сортировки.
	int total=0;
	int survivor[MAXPLAYERS + 1];
	for(int client = 1; client <= MaxClients; client++)
	{
		if( IsClientInGame(client) && !IsFakeClient(client) && GetClientTeam(client) == 2 )
		{
			if( IsPlayerAlive(client) && IsClientObserver(client) )//Убрать зрителей.
				continue;
	...
	//Массив survivor[players] сортируется с использованием функции killinfo (функции подсчета убийств)
	SortCustom1D(survivor,sizeof(survivor),SortKillInfo);
	HUD hud_playerlist;
	for(int i = 0; i < total; i++)
	{
		// Максимум - 8. Больше мест нет.
	...
	while( total < 8 ) // Удалить лишний слот, если количество игроков уменьшилось
	{
		RemoveHUD(total + 2);
		total++;
	}
}
//Статистика убийств ведётся с использованием события playerdeath
void Event_PlayerDeathInfo(Event event, const char[] name, bool dontBroadcast)
{
	...
	if( attacker < 0 || attacker > MaxClients || !IsClientInGame(victim) ) //поскольку attacker = 0 указывает на то, что причиной смерти является мир
		return;

	event.BroadcastDisabled = true; //Используя prehook, мы задаём это значение, чтобы отключить отображение информации об убийстве красным шрифтом.

	if( !(g_show_list & KILL_LIST) )
		return;

	static char killinfo[128];
	if( attacker == 0 ) //Умер от падения
	{
	...
	if( GetClientTeam(attacker) == 3 && GetClientTeam(victim) == 2 ) //Убийство особыми заражёнными
	{
	...
	{
		g_player_killspecial[attacker] += 1; //Убийце: +1
	}
	...
	{
		FormatEx(killinfo,sizeof(killinfo),"    %s  %s",g_kill_type[11],victim_name);
		DisplayKillList(killinfo);
		return;
	}
	...
	if( event.GetBool("headshot") )
	{
		...
		if( IsKilledBehindWall(attacker,victim) )
			FormatEx(killinfo,sizeof(killinfo),"%N  %s %s %s  %s",attacker,g_kill_type[13],g_kill_type[14],weapon_type,victim_name);
		else
	...
	{
		if( IsKilledBehindWall(attacker,victim) )
			FormatEx(killinfo,sizeof(killinfo),"%N  %s %s  %s",attacker,g_kill_type[13],weapon_type,victim_name);
		else
